...
Problems will be fixed quicker if there is a clear procedure to how to fix them.
...
Same artifact built twice may behave differently in the same conditions.
...
When a function call crash we get a stacktrace with all the calls from start to finish.
